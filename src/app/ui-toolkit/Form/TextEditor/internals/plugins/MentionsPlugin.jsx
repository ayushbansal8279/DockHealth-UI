...
export default function MentionsPlugin({
  ...
  const { results, avatars } = useMentionLookupService(
    taskListIdentifier,
    queryString,
  );
  ...
  const fetchUsersWithDebounce = debounce((value) => {
  ...
  return (
    <LexicalTypeaheadMenuPlugin
      ...
      menuRenderFn={(
        ...
        anchorElementReference && results.length > 0
          ? ReactDOM.createPortal(
              <div className="typeahead-popover mentions-menu">
                <Paper sx={{ width: 230 }}>
                  <MenuList>
                    {options.map((option, i) => {
                      const mention = new MentionTypeaheadOption(
                        option.identifier,
                        option.name,
                        avatars[option.identifier],
                        option.bubbleColor,
                      );
                      return (
