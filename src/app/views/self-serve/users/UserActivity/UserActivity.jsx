...
const UserActivity = () => {
    ...
    const fetchDataAndRender = async () => {
        const fromDateTime = moment().subtract(1, 'months').format('YYYY-MM-DD HH:mm:ss');
        const toDateTime = moment().format('YYYY-MM-DD HH:mm:ss');
        try {
        const userDetailsResponse = await getUserById(userIdentifier);
        setUserDetails(userDetailsResponse);
        const response = await getUserActivity(userIdentifier, fromDateTime, toDateTime);
    ...
    const defaultColumns = [
    {
        ...
        width: 250,
        renderCell: ({ row }) => {
        const formattedDate = `${moment(row.eventDateTime).format('MMM DD, YYYY')} @ ${moment(row.eventDateTime).format('hh:mm a')}`;
        return (
    ...
    return (
        ...
        <UserName>
        {[`${userDetails?.lastName},`, userDetails?.firstName, userDetails?.middleName].join(' ')}
        </UserName>
        </BackButtonContainer>
        ...
        <Grid container size={12} item justifyContent="center" sx={{ paddingTop:'5px'}}>
            <Grid item size={12} xl={11} md={12} lg={11} sx={{ display: 'flex', justifyContent: 'center' }}>
                <StyledDataGrid
                    ...
                    headerHeight={35}
                    getRowHeight={(row) => { return (row?.model?.eventAction?.length > 55) ? 'auto' : 40 }}
                    pagination
