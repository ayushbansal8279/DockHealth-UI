...
const Item = React.forwardRef(({ children, ...props }: any, ref) => (
  <div ref={ref} className={cx.Item} {...props}>
    <div>
      {props.item.name} <i>{props.item.details}</i>{' '}
      <b>
...
const invokeUltimateFetcher = (offset: number = 0, limit: number = 10) => {
  ...
  return Promise.all(fetches)
    ...
    .then((tasks) => {
      ...
      return Promise.all(tasksWithSubtasksPromises)
        .then((tasksWithSubtasks) => {
          const tasksWithSubtasksIds = tasksWithSubtasks.map(
            (tasksWithSubtask) => tasksWithSubtask.identifier,
          );
          return tasks.map((task: any) => {
            if (tasksWithSubtasksIds.includes(task.identifier)) {
              return tasksWithSubtasks.find(
                (tasksWithSubtask) =>
                  tasksWithSubtask.identifier === task.identifier,
              );
            }
            return task;
          });
...
export default function Virtualized() {
  ...
  const loadMore = useCallback(
    debounce((item2) => {
      ...
      invokeUltimateFetcher(offset, offset).then((d) => {
