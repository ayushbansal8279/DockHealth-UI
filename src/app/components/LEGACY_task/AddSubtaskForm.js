...
import { change, destroy, Field, reduxForm, reset } from 'redux-form';
...
class AddSubtaskForm extends Component {
  ...
  componentWillUpdate(nextProps) {
    if (
      this.props.currentSubtask &&
      this.props.currentSubtask != this.state.subtask
    ) {
      this.setState({ subtask: this.props.currentSubtask });
      this.setState({
        priorityFlag: this.props.currentSubtask.priority === 'HIGH',
      });
    }
  }
  ...
  changeTaskPriority = () => {
    const priorityFlag = !this.state.priorityFlag;
    this.setState({ priorityFlag });
    ...
    const renderDescriptionField = ({
      ...
      >
        ...
        <div className="input-group icon-right icon-left">
          ...
          <span
            onClick={() => this.changeTaskPriority()}
            className="input-group-label"
          >
            <svg
              className={`icon flag medium ${
                this.state.priorityFlag ? '' : 'no-flag'
              }`}
            >
        ...
        <div className="subtask-wrapper inline-label">
          <div className="row expanded">
            <div className="column small-4 toggle-add-subtask">
              <svg onClick={e => this.showState()} className="icon">
          ...
          <div className="column large-12 input-group has-value">
            <span className="task-title">
              {`Task: ${
                this.props.currentTask && this.props.currentTask.description
          ...
          {this.props.title != 'Inbox' || this.props.taskListSelection ? (
            <span>
              <Field
                id="assign-subtask-to"
                name="assignedTo"
                type="text"
                component={BasicField}
                label="Assigned to"
                xlinkHref="#icon-assign-to"
                extraClassName="assign-subtask-to"
              />
              <Field
          ...
          <Field
            ...
            extraClassName="duedatepickdate"
          />
