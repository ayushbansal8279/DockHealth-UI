...
const ProfileNote = ({
  ...
  const {
    identifier: profileNoteIdentifier,
    dateCreated,
    dateUpdated,
    creator,
    lastEditor,
  ...
  const menuOptions = useMemo(() => {
    const options = [
      {
        ...
        onClick: () => onPinChange(profileNoteIdentifier, !pinned),
      },
    ];
    ...
    if (isEditable) {
      ...
      options.push({
        name: 'Delete',
        onClick: () => onRemove(profileNoteIdentifier),
        color: palette.error,
      });
    }
    return options;
  }, [isEditable, onPinChange, onRemove, profileNoteIdentifier, pinned]);
