  ...
  const mergePatientUpdateAndMetaActivities = useCallback((activities) => {
    ...
    for (let i = 0; i < activities.length; i++) {
      ...
      const isPatientMetaMerge =
        ...
        ((current.actionType === 'UPDATE_PATIENT' &&
          next.actionType === 'SAVE_PATIENT_META_DATA') ||
          (current.actionType === 'SAVE_PATIENT_META_DATA' &&
            next.actionType === 'UPDATE_PATIENT'));
  ...
  const sortedActivities = useMemo(() => {
    ...
    for (const activity of merged) {
      if (activity?.actionType === 'CREATE_ATTACHMENT_PATIENT') {
        const key = format(
          parseISO(activity.activityDateTime),
          'yyyy-MM-dd HH:mm',
        );
    ...
    const groupedAttachments = Array.from(attachmentsMap.values()).map(
      (group) =>
        group.length === 1
          ? group[0]
          : {
    ...
    return allActivities
      ...
      .filter(
        (act) =>
          ...
          selectedFilters.includes(act.targetType),
      );
