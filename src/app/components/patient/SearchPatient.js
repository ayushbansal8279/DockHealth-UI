...
class SearchPatient extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      searchToken: '',
      ...
      <form className="inline-label" onSubmit={handleSubmit(this.onSubmit)}>
            ...
            <div className="columns input-group input-wrapper bottom-buffer-small icon-right">
              <div className="input-wrapper">
                <input
                  ...
                  type="text"
                  placeholder="Search patient in EMR"
                  onChange={this.onChange.bind(this)}
                  onKeyPress={this.handleKeyPress}
                />
              </div>
              <span
                className="input-group-label pointer"
        ...
        {this.props.isFetching ? (
          ...
          <div className="column large-12 search-result-wrapper">
            {this.props.emrPatients &&
              this.props.emrPatients.map((patient, index) => {
                return (
                  <div
                    className="item row expanded align-middle"
                    onClick={this.selectPatient.bind(this, patient)}
                    key={`mrn_${patient.mrn}_${index}`}
                  >
